...
const symbolsCount = (post) => {
    ...
    if (symbolsResult > 9999) {
        symbolsResult = Math.round(symbolsResult / 1000) + 'k chars'; // > 9999 => 11k chars
    } else if (symbolsResult > 999) {
        symbolsResult = Math.round(symbolsResult / 100) / 10 + 'k chars'; // > 999 => 1.1k chars
    } else {
        symbolsResult = symbolsResult.toString() + ' chars'; // < 999 => 111 chars
    }
...
const symbolsCountTotal = (site) => {
    ...
    return symbolsResultTotal < 10000
        ? Math.round(symbolsResultTotal / 1000) + 'k chars'
        : Math.round(symbolsResultTotal / 1000) / 10 + 'k chars';
};
